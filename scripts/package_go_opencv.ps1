...
<#
.SYNOPSIS
  Build example/example_opencv.go into go_opencv/, copy OpenCV/MinGW DLLs to go_opencv/lib/.

.DESCRIPTION
  Windows does not load DLLs from .\lib automatically. Use go_opencv\run_example_opencv.bat (prepends lib to PATH).

.PARAMETER BundleRoot
  Default: <repo>/go_opencv

.EXAMPLE
  .\scripts\package_go_opencv.ps1 -OpenCVBin "D:\opencv\build\x64\mingw\bin" -MingwBin "C:\msys64\mingw64\bin"
#>
[CmdletBinding()]
param(
    [string] $BundleRoot = "",
    ...
    [string] $MingwBin = "",
    [string[]] $ExtraDllDirs = @(),
    [string] $MainRel = "example/example_opencv.go",
    [string] $ExeName = "example_opencv.exe"
)
...
if (-not $BundleRoot) {
    $BundleRoot = Join-Path $repoRoot "go_opencv"
}
...
Write-Host "==> Resolve exe dependencies -> go_opencv\lib"
...
$runBat = Join-Path $bundleFull "run_example_opencv.bat"
$runLines = @(
    '@echo off',
    'setlocal',
    'chcp 65001 >nul',
...
[System.IO.File]::WriteAllText($runBat, ($runLines -join "`r`n"), [System.Text.ASCIIEncoding]::new())

Write-Host "==> Done. Run: go_opencv\run_example_opencv.bat   (or set PATH=%CD%\lib;%PATH% then run the exe)"
