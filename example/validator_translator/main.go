...
import (
	...
	"github.com/ifengshai/go_utils/tools"
...
func main() {
	...
	if v, ok := binding.Validator.Engine().(*validator.Validate); ok {
		// 添加额外自定义验证方法报错的翻译
		_ = v.RegisterTranslation("notadmina", tools.Trans, func(ut ut.Translator) error {
	...
	r.GET("/bookable", func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, tools.ErrResp(err)) //这里调用翻译
			return
		}
